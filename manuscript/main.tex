...
The synthetic data in all scenarios use a linear ground truth: hazard depends on $\bbeta^\top \bx$ without interaction terms or nonlinear transformations. The double descent patterns we observe therefore reflect the difficulty neural networks have in recovering a linear relationship, not their capacity to model complex nonlinear survival dynamics. Whether double descent occurs when the true data-generating process requires nonlinear modeling remains untested.
