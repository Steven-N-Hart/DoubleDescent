...
With twenty random seeds per configuration, the reported standard errors are substantial (0.031--0.116 for concordance), and notably larger than observed with fewer seeds, limiting statistical power to detect small effects and suggesting higher variability in individual runs. The claimed regularization benefit of approximately 2\% absolute concordance improvement does not reach statistical significance at $\alpha = 0.05$ based on paired testing across seeds.
